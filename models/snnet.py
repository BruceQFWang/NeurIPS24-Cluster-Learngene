...
from utils.utils import get_stitch_configs, ps_inv
    ...
    def initialize_stitching_weights(self, x):
        ...
        for i, combo in enumerate(self.candidate_combinations):
            ...
            for stitch_layer_id, stitch_positions in stitching_dicts.items():
                ...
                print(f'Initialized Stitching Model {front} to Model {end}, Layer {stitch_layer_id}')
...
class SNNet(nn.Module):
    def __init__(self, anchors, in_dim, out_dim):
        super(SNNet, self).__init__()
        self.anchors = anchors
        self.stitch_layer = StitchingLayer(in_dim, out_dim)

    def forward(self, x):
        return self.stitch_layer(x)

    def initialize_stitching_weights(self, x):
        with torch.no_grad():
            front_features = self.anchors[0].extract_block_features(x)
            end_features = self.anchors[1].extract_block_features(x)

        weight_candidates = []
        bias_candidates = []

        for front_id, end_id in zip(range(len(front_features)), range(len(end_features))):
            front_blk_feat = front_features[front_id]
            end_blk_feat = end_features[end_id - 1]
        ...
        self.stitch_layer.init_stitch_weights_bias(weights, bias)
        print(f'Initialized Stitching Layer')


class ExtendedBlocks(nn.Module):
    def __init__(self, blocks):
        super(ExtendedBlocks, self).__init__()
...
class ExtendedViT(nn.Module):
    def __init__(self, vit_model, snnet, learngene_depth):
        super(ExtendedViT, self).__init__()
        self.vit_model = vit_model
        self.snnet = snnet
        self.learngene_depth = learngene_depth
        embed_dim = self.vit_model.blocks[-1].mlp.fc2.out_features
        # Classifier head
        # self.head = vit_model.head

    def forward(self, x, ada_token=None, use_patches=False):
        # Preprocessing step
        x = self.vit_model.prepare_tokens(x, ada_token)

        # Pass input through the first part of the ViT model
        for i in range(self.learngene_depth):
            x = self.vit_model.blocks[i](x)
        
        # Pass the output of the first part through the SNNet
        x = self.snnet(x)
        # Pass the output through the second part of the ViT model
        for i in range(self.learngene_depth, self.learngene_depth*2):
            x = self.vit_model.blocks[i](x)
        #print(self.vit_model)
        #print(self.learngene_depth*2)
        #exit()
